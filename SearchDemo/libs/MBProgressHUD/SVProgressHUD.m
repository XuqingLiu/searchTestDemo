...
- (void)updatePosition {
    ...
    if(string) {
        CGSize stringSize = [string adaptSizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(MAIN_SCREEN_WIDTH - 100, 300) lineBreakMode:NSLineBreakByCharWrapping];
        stringWidth = stringSize.width;
        stringHeight = stringSize.height;
        if (imageUsed)
            hudHeight = 80+stringHeight;
        else
            hudHeight = 16+stringHeight;
        
        if(stringWidth > hudWidth)
            hudWidth = ceil(stringWidth/2)*2;
        
        CGFloat labelRectY = imageUsed ? 66 : 8;
