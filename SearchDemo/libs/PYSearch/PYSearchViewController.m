...
- (void)setup
{
    ...
    self.searchHistoryStyle = PYSearchHistoryStyleColorfulTag;
...
- (void)pushToVCWithText:(NSString *)str {
    //转移页面，发送请求
    MXListViewController *userListVC = [[MXListViewController alloc] init];
    userListVC.userStr = str;
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    if ([self.delegate respondsToSelector:@selector(searchViewController:didSearchWithSearchBar:searchText:)]) {
        ...
        [self saveSearchCacheAndRefreshView];
        [self pushToVCWithText:searchBar.text];
        return;
    }
